...
from sounds_and_music import play_music, set_current_music
...
def check_background(player, map):
    ...
    if map[player.x][player.y - 1] == (ICE_bg or ICE):
        if previous_bg != ICE_CAVE_BG_img:
            set_current_music(SNOW_BIOM_SOUNDTRACK_PATH)
            play_music()
        ...
        if previous_bg != BACKGROUND_img:
            ...
            monster_x = 120
            monster_y = 51
            set_current_music(DEFAULT_BIOM_SOUNDTRACK_PATH)
            play_music()
        return BACKGROUND_img
    elif map[player.x][player.y - 1] == (GROUND_bg or GROUND):
        if previous_bg != GROUND_CAVE_BG_img:
            set_current_music(CAVE_BIOM_SOUNDTRACK_PATH)
            play_music()
